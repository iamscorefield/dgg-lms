...
import { Search, ShieldCheck, ShieldAlert, ShieldQuestion, Calendar, Award, FileCheck, ExternalLink, Printer, Mail, UserCheck, CheckCircle, Users, MessageSquare } from "lucide-react";
    ...
    try {
      ...
      if (!adminError && adminData) {
        ...
        setResult({
          ...
          avgScore: Number(adminData.average_score || 85.00),
          ...
          attendanceRate: Number(adminData.attendance_rate || 100.00),
          weeklyBreakdown: adminData.weekly_breakdown || "Weekly breakdown logs synced.",
          disciplineComment: adminData.discipline_comment || "Maintained high professional conduct throughout the term.",
          teamworkComment: adminData.teamwork_comment || "Demonstrated excellent collaboration during interactive cohort exercises.",
          hostComment: adminData.host_comment || "Consistent participation and strong grasp of core lab directives.",
          ceoComment: adminData.ceo_comment || "Approved and endorsed for placement in professional tech environments."
        });
        setLoading(false);
        return;
      }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 print:px-0">
        ...
        <div className="text-left mb-10 print:hidden">
          ...
          <p className="text-sm font-medium text-slate-500 mt-2 leading-relaxed">
            Validate authentic graduation records, transcripts, and stakeholder recommendations instantly. Input a unique <strong>Certificate Serial Number</strong> or <strong>Registered Email</strong>.
          </p>
        </div>
            ...
            {accountStatus === "certified" && result && (
              <div className="border-2 border-slate-200/80 rounded-3xl bg-white overflow-hidden shadow-sm relative print:border-0 print:shadow-none">
                ...
                <div className="p-6 sm:p-10 space-y-8">
                  ...
                  {/* ========================================================= */}
                  {/* NEW RECRUITER DOSSIER: ATTENDANCE & BEHAVIORAL EVALUATION */}
                  {/* ========================================================= */}
                  {result.source === 'admin' && (
                    <div className="space-y-4 pt-2 border-t border-slate-100">
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                        <Users size={14} className="text-[#512d7c]" /> Professional Transcript &amp; Behavioral Audit Dossier
                      </h3>
                      
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="border border-slate-200 bg-slate-50/50 p-4 rounded-2xl space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Overall Attendance Rate</span>
                          ...
                          <p className="text-xs font-bold text-slate-700 mt-1">{result.weeklyBreakdown}</p>
                        </div>

                        <div className="border border-slate-200 bg-slate-50/50 p-4 rounded-2xl space-y-1">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Discipline &amp; Punctuality</span>
                          <p className="text-xs font-medium text-slate-700 leading-relaxed mt-1">{result.disciplineComment}</p>
                        </div>

                        <div className="border border-slate-200 bg-slate-50/50 p-4 rounded-2xl space-y-1">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Teamwork &amp; Collaboration</span>
                          <p className="text-xs font-medium text-slate-700 leading-relaxed mt-1">{result.teamworkComment}</p>
                        </div>

                        <div className="border border-slate-200 bg-slate-50/50 p-4 rounded-2xl space-y-1">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Host / Instructor Engagement</span>
                          <p className="text-xs font-medium text-slate-700 leading-relaxed mt-1">{result.hostComment}</p>
                        </div>
                      </div>
                      ...
                      <div className="border border-purple-200 bg-purple-50/40 p-5 rounded-2xl space-y-1.5 text-left">
                        <div className="flex items-center gap-1.5 text-[#512d7c] font-black text-xs uppercase tracking-wider">
                          <MessageSquare size={14} /> Executive CEO &amp; Founder Recommendation
                        </div>
